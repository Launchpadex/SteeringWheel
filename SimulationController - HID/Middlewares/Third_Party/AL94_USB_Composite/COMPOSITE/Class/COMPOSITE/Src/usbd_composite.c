...
#endif
#if (USBD_USE_HID2_CUSTOM == 1)
  uint8_t USBD_HID2_CUSTOM_DESC[USB_CUSTOM_HID2_CONFIG_DESC_SIZ - 0x09];
#endif
...
#endif
#if (USBD_USE_HID2_CUSTOM == 1)
  USBD_HID2_CUSTOM.Init(pdev, cfgidx);
#endif
#if (USBD_USE_UAC_MIC == 1)
...
#endif
#if (USBD_USE_HID2_CUSTOM == 1)
  USBD_HID2_CUSTOM.DeInit(pdev, cfgidx);
#endif
#if (USBD_USE_UAC_MIC == 1)
                                    ...
                                    USBD_SetupReqTypedef *req)
{
  ...
  {
    return USBD_HID2_CUSTOM.Setup(pdev, req);
  }
#endif
#if (USBD_USE_UAC_MIC == 1)
...
static uint8_t USBD_COMPOSITE_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  {
    return USBD_HID2_CUSTOM.DataIn(pdev, epnum);
  }
#endif
#if (USBD_USE_UAC_MIC == 1)
...
#endif
#if (USBD_USE_HID2_CUSTOM == 1)
  USBD_HID2_CUSTOM.EP0_RxReady(pdev);
#endif
#if (USBD_USE_UAC_MIC == 1)
...
static uint8_t USBD_COMPOSITE_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  {
    return USBD_HID2_CUSTOM.DataOut(pdev, epnum);
  }
#endif
#if (USBD_USE_UAC_MIC == 1)
#endif
...
#if (USBD_USE_HID2_CUSTOM == 1)
  ptr = USBD_HID2_CUSTOM.GetFSConfigDescriptor(&len);
  USBD_Update_HID2_Custom_DESC(ptr, interface_no_track, in_ep_track, out_ep_track, USBD_Track_String_Index);
  memcpy(USBD_COMPOSITE_FSCfgDesc.USBD_HID2_CUSTOM_DESC, ptr + 0x09, len - 0x09);

  ptr = USBD_HID2_CUSTOM.GetHSConfigDescriptor(&len);
  USBD_Update_HID2_Custom_DESC(ptr, interface_no_track, in_ep_track, out_ep_track, USBD_Track_String_Index);
  memcpy(USBD_COMPOSITE_HSCfgDesc.USBD_HID2_CUSTOM_DESC, ptr + 0x09, len - 0x09);
